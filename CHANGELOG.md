...
- **CI: dist reproducibility** — новий крок у `local-windows-validation.yml` порівнює закомічений `dist/Get-BravoSystemReport.ps1` зі свіжою збіркою з `src/` через нормалізоване порівняння вмісту (рядок `GeneratedAt` — недетермінований timestamp за дизайном — виключається з обох боків, CRLF/trailing whitespace нормалізуються; початкова реалізація через `git diff --exit-code -- dist/` структурно падала б на кожному прогоні саме через цей timestamp).
- **CI: цілісність dist тепер перевіряється трьома незалежними кроками** (acceptance-review fix): (1) закомічені runtime і `.sha512` звіряються між собою ДО build-етапу (раніше build перезаписував обидва файли до перевірки, і закомічений `.sha512` — саме той, яким кінцевий користувач звіряє завантажений dist — не валідувався взагалі); (2) свіжа збірка перевіряється на внутрішню SHA512-консистентність після build; (3) закомічений runtime порівнюється зі свіжим після нормалізації `GeneratedAt`. Порівняння захищено від OEM-кодових сторінок консолі (`[Console]::OutputEncoding = UTF8` перед `git show`-захопленням кириличного UTF-8 вмісту).
- **Storage: канонічна класифікація партицій `Resolve-BravoPartitionType`** (acceptance-review fix) — `Get-Partition.Type` не мапить MBR WinRE (`MbrType 0x27`) у `Recovery` на legacy-BIOS машинах, тож штатно заповнений на 90%+ WinRE-розділ потрапляв би у звичайний аналіз вільного місця (систематичний false CRITICAL + exit 4 у `-Strict` на MBR-парку). Нова чиста функція нормалізує тип за прецедентністю GPT GUID (EFI System/MSR/Recovery) → MBR `0x27` → `IsSystem` → наявний `Type` → `Unknown`; кореляція том↔партиція отримала fallback за volume GUID path (`AccessPaths`)/літерою диска (без heuristic-збігу за розміром); folder-mounted data-томи (`Basic`) як і раніше НЕ Reserved. Некорельований том без літери з малим вільним місцем більше не породжує неправдивий CRITICAL — INFO-знахідка `Storage.UnknownVolume` + виключення з capacity-аналізу (видимий у звіті, Health Score не страждає без доказу data-тому).
- **HTML footer: реальний OutputDir більше не витікає при `-Sanitize`** (acceptance-review fix, той самий клас, що й маскування `Report.OutputPath` вище) — footer брав сирий параметр `$OutputDir` в обхід report-моделі і був єдиним місцем реальної ідентичності (шлях з username) у sanitized HTML; тепер використовує `Report.OutputPath` (замаскований санітизацією). Регресія: негативний скан HTML на literal шлях каталогу звітів у `tests/ExecutionContract.Tests.ps1`.
