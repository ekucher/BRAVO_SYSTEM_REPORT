...
- `push` у `main`, `developer`, `bravo/integration/modular-build` або `release/**`;
- ручний `workflow_dispatch` на вибраній maintainer-ом ревізії.

Тобто Windows integration validation спрацьовує вже після merge/push у одну з цих гілок
або за ручним запуском, а не на самому pull request.

Перевірки:

- `git diff --check`;
- перевірка committed `dist/Get-BravoSystemReport.ps1.sha512` до перебудови (виявляє
  незакомічену або застарілу перебудову dist);
- build modular monolith;
- перевірка SHA512 і reproducibility згенерованого `dist` проти committed версії;
- PowerShell parser check для `dist`;
- повний Pester-набір (`tests/`);
- Quick runtime test + JSON validation (`Profile=Quick`, `CollectionErrors=0`);
- Full runtime test і валідація секції `Updates`;
- перевірка, що у tracked files немає випадково закомічених публічних IPv4 literals;
- Deep runtime test (CSV + ZIP);
- exit-code contract test (форсована export-помилка → exit code `1`, без `CollectionErrors`).

### Static/hosted PR checks (без self-hosted залежності)

```text
.github/workflows/powershell-static-check.yml
```

Запускається на PR у `main`/`developer`/`release/**`, push у `developer` і вручну
...
contract change guard (`SchemaVersion`+`CHANGELOG.md` при зміні `20-ReportModel.ps1`,
лише на PR), version consistency (`ScriptVersion`/`SchemaVersion` vs README/ROADMAP),
Windows-незалежний підмножина Pester (`SourceParserCheck`, `ParameterSurface`). Усе, що
