...
- **Release candidate**: ScriptVersion 0.6.1 (SchemaVersion 0.6.20). Гілка `release/v0.6.1-stable` (PR #85 → `main`, відкритий, не змержений) — вихідний RC. **Активна робота перенесена на `integration/v0.6.1-secure`** (розгалужена від `release/v0.6.1-stable`, включає всі його коміти плюс окремий цикл стабілізації: усунення self-hosted CI runner exposure на `pull_request` (Issue #89), дисposition усіх 25 тредів review-бота PR #85, приватність/цілісність Sanitize fail-closed фікси, реконсиляція GitHub Issues #89-#100 проти живого коду (замість сліпого прийняття їхнього тексту), розширений Pester-набір (27 файлів / 316 тестів), unpack-and-run release smoke test, документаційна звірка). PR #85 у своєму поточному вигляді НЕ відображає цю роботу — фінальний release-promotion PR у `main` буде підготовлено з `integration/v0.6.1-secure` окремим кроком (ще не створений).
...
- [x] UAC full policy (`HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System`, `src/34-Collectors-Security.ps1`; `Security.UAC.{ConsentPromptBehaviorAdminCode,ConsentPromptBehaviorAdminText,ConsentPromptBehaviorUserCode,ConsentPromptBehaviorUserText,PromptOnSecureDesktop,FilterAdministratorToken}`; дві нові чисті функції `Get-BravoUacAdminPromptText`/`Get-BravoUacUserPromptText` мапують DWORD-код у людяний опис, покриті 6 unit-тестами; WARNING лише для найризикованішого `ConsentPromptBehaviorAdmin=0` — "Elevate without prompting"; відсутність налаштування — штатний дефолт групової політики, не помилка).
...
- [x] Autoruns (`src/34-Collectors-Security.ps1`, гейтовано окремо Deep/Forensic, не Full — більший обсяг даних; `Security.Autoruns[]` — Name/Command/Source/Hive; реєстрові ключі Run/RunOnce у HKLM/HKCU + Wow6432Node на 64-bit, плюс папки автозавантаження User/AllUsers Startup; нова чиста функція `ConvertFrom-BravoRegistryKeyProperties` відкидає службові PS*-метавластивості `Get-ItemProperty`, покрита unit-тестами).
- [x] Scheduled tasks (`Get-ScheduledTask`, `src/34-Collectors-Security.ps1`, гейтовано Deep/Forensic; `Security.ScheduledTasks[]` — Name/Path/State/Author/Execute/Arguments/IsMicrosoftDefault; вбудовані `\Microsoft\Windows\*` задачі (типово 200-300+ на машині) позначаються прапорцем `IsMicrosoftDefault`, не приховуються з моделі — той самий принцип "дані видимі, findings обережні", що й ARP/Storage ReservedVolumes; HTML-таблиця за замовчуванням показує лише не-Microsoft задачі, з лічильником прихованих).
...
**Актуалізовано 2026-09-02**: цей розділ був написаний до фактичної реалізації — усі пункти Етапів 1-4 і acceptance criteria вже реалізовані в `src/51-Export-Html.ps1` (перевірено звіркою з кодом), позначені `[x]` заднім числом. **Розділ v0.6.1 тепер повністю закритий**, включно з усім Етапом 5 (живий пошук/фільтрація, друковані CSS-стилі, Dark Mode, Edge CLI PDF).

### Етап 1. Архітектура даних та підготовка бекенду PowerShell

- [x] Рефакторинг об'єкта `$Report`:
  - [x] структурувати дані так, щоб вони логічно відповідали майбутнім вкладкам: `OS`, `Hardware`, `Network`, `Security`, `Services`, `Software` — усі присутні як топ-рівневі секції моделі (`src/20-ReportModel.ps1`).
- [x] Виправлення порожніх та обмежених метрик:
  - [x] чесний розрахунок відсотка утилізації RAM (`Hardware.RAM.UsedPercent`, `src/31-Collectors-Hardware.ps1`, реальне ділення Used/Total, не заглушка);
  - [x] ліміту `Select-Object -First 50` для встановленого ПЗ немає в коді (перевірено `grep` по `src/38-Collectors-Software.ps1`);
  - [x] збір шлюзів `Gateway` для мережевої вкладки (`Network.Routing.DefaultGateways`);
  - [x] збір DNS-серверів для мережевої вкладки (`Network.Routing.DNSServers`).
- [x] Підготовка контенту для індикаторів стану Health Status:
  - [x] `$Report.Status`/`$Report.StatusReason` присутні та узгоджені з `$Report.Health.Status` (`src/40-Health.ps1`).

### Етап 2. Проєктування UI/UX шаблону HTML / CSS

- [x] Верстка каркасу Layout: фіксований `.dashboard-header` з назвою ПК, uptime (`Dashboard.Header.UptimeText`), health/status pill, датою формування звіту (`Timestamp`), блок навігації `.tab-button`.
- [x] Дизайн головного дашборду General Tab: `.metrics-grid`/`.metric-card` для CPU/RAM/Disk/OS (`Dashboard.Metrics`), колірне кодування через `.status-ok`/`.status-warning`/`.status-critical` класи.
- [x] Верстка контенту для вкладок: універсальний `.data-table`, `.table-scroll` з `max-height`+`overflow:auto` для великих масивів (ПЗ, процеси, event logs).

### Етап 3. Інтеграція логіки перемикання JavaScript

- [x] Таборування на Vanilla JS: `window.openTab(event, tabId)` приховує/показує `.tab-panel` через клас `.active`, керує `.tab-button.active` (`src/51-Export-Html.ps1`, вбудований `<script>`).
- [x] Оптимізація та автономність: увесь JS/CSS інлайн у HTML-шаблоні, без зовнішніх файлів/CDN — відповідає ключовій вимозі розділу (звіт відкривається offline).

### Етап 4. Динамічна збірка звіту в PowerShell

- [x] HTML-код загорнутий у PowerShell string-інтерполяцію (`src/51-Export-Html.ps1`), генерація даних (рядки таблиць) відокремлена в `$xxxRows`-змінні перед вставкою в розмітку.
- [x] Цикли для масивів даних: `foreach`/`ForEach-Object` генерують рядки таблиць для ПЗ, дисків, мережевих адаптерів, процесів, служб, event logs.

### Етап 5. Просунутий функціонал, фічі на майбутнє

- [x] Живий пошук / фільтрація: `New-BravoTableToolbarHtml` + `filterTable()` JS — поле пошуку над КОЖНОЮ таблицею (не лише ПЗ; уже покриває процеси/служби/event logs/усі v0.5.0 таблиці), генерично.
- [x] Друковані CSS-стилі: `@media print` приховує `.tab-nav`/`.table-toolbar`, розгортає всі `.tab-panel` й `.table-scroll` для друку на A4.
- [x] Автоматична конвертація HTML у PDF через Edge CLI (`msedge --headless --print-to-pdf`) — новий опційний параметр `-ExportPdf` (`src/05-Params.ps1`, форвардиться через wrapper і elevation-relaunch); `Get-BravoEdgeExecutablePath`/`Export-BravoPdfReport` (`src/51-Export-Html.ps1`) — шукає `msedge.exe` через PATH і два стандартних шляхи встановлення (64-bit і 32-bit під Program Files (x86)); відсутність Edge на машині — НЕ помилка збору/експорту, лише `[INFO]` у консоль, опційна фіча; PDF додається в `GeneratedFiles` до ZIP-пакування, якщо конвертація вдалась.
- [x] Темна тема Dark Mode (`src/51-Export-Html.ps1`): CSS-змінні для card/nav/table поверхонь (`--panel`, `--nav-bg`, `--btn-bg`, `--tab-panel-bg`, `--th-bg` тощо), `@media (prefers-color-scheme: dark)` як дефолт без явного вибору + `:root[data-theme="dark"]`/`[data-theme="light"]` override для явного перемикання; кнопка `theme-toggle` в header, JS `toggleTheme()`; вибір теми зберігається через `localStorage` з try/catch — на `file://` протоколі деякі браузери можуть обмежувати доступ, тоді тема перемикається лише в межах поточної сесії перегляду, без збереження між відкриттями файлу (не критично).

### Acceptance criteria для Dashboard & Tabs

- [x] HTML-звіт відкривається без інтернету.
- [x] Усі CSS і JS вбудовані в HTML.
- [x] Є вкладки General, OS, Hardware, Network, Security, Services, Software (і ширше — Storage/Users окремо винесені як додаткові вкладки понад мінімальний список).
- [x] Вкладки перемикаються без перезавантаження сторінки.
- [x] Активна вкладка має видимий `.active` стан.
- [x] General Tab містить метричні картки CPU, RAM, Disk, OS.
- [x] Великі таблиці не розтягують сторінку безконтрольно, а мають scroll-контейнери.
- [x] Встановлене ПЗ не обрізається штучним `Select-Object -First 50`.
- [x] Gateway і DNS відображаються в Network tab.
- [x] Health/status показується у Header і General Tab.
