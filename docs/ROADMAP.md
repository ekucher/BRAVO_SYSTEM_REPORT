...
**Актуалізовано 2026-09-02**: цей розділ був написаний до фактичної реалізації — усі пункти Етапів 1-4 і acceptance criteria вже реалізовані в `src/51-Export-Html.ps1` (перевірено звіркою з кодом), позначені `[x]` заднім числом. З Етапу 5 реалізовано лише живий пошук/фільтрація (генерично для всіх таблиць, не лише ПЗ) і друковані CSS-стилі; Dark Mode і автоматична конвертація в PDF через Edge CLI — не реалізовані, лишаються в бэклозі.
...
- [ ] Темна тема Dark Mode — не реалізовано (немає `prefers-color-scheme`/перемикача теми/CSS-змінних для темної палітри).
