...
**Актуалізовано 2026-09-02**: цей розділ був написаний до фактичної реалізації — усі пункти Етапів 1-4 і acceptance criteria вже реалізовані в `src/51-Export-Html.ps1` (перевірено звіркою з кодом), позначені `[x]` заднім числом. З Етапу 5 реалізовано живий пошук/фільтрація, друковані CSS-стилі та Dark Mode; лишається лише автоматична конвертація в PDF через Edge CLI.
...
- [ ] Автоматична конвертація HTML у PDF через Edge CLI (`msedge --headless --print-to-pdf`) — не реалізовано, CSS для друку є, автоматизації виклику Edge немає.
