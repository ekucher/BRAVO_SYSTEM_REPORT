...
- Маскування чутливих даних через `-Sanitize`/`-SanitizeLevel Basic|Strict` — реалізовано (`src/45-Sanitize.ps1`), fail-closed: збій маскування посередині проходу зупиняє весь export (жоден файл не пишеться), а не лише той, що маскувався в момент збою (див. `README.md` розділ "Sanitize", `docs/ROADMAP.md`).
...
- **Exit code contract**: `0` — успіх, без помилок; `1` — завершено, але були `CollectionErrors`/`ExportErrors`; `2` — фатальна неопрацьована помилка (top-level `trap` у `90-Main.ps1`); `3` — обов'язковий JSON не згенеровано; `4` — лише з `-Strict`: без `CollectionErrors`/`ExportErrors`, але `Health.Status` = `CRITICAL`; `5` — лише з `-Sanitize`: маскування провалилось посередині (fail-closed), жоден звіт не записано. Health Status (WARNING/CRITICAL) за замовчуванням (без `-Strict`) НЕ впливає на exit code — Health описує стан аудитованої машини, а не збій самого інструмента. Деталі: `README.md` розділ "Exit code contract".
